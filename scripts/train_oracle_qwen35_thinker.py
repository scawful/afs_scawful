#!/usr/bin/env python3
"""Train a named Zelda oracle persona on the shared Qwen 3.5 thinker lane."""
...
TRACK_NAME = os.environ.get("TRAIN_TRACK_NAME", "oracle-qwen35-thinker")
PERSONA_NAME = os.environ.get("TRAIN_PERSONA_NAME", "oracle")
MODEL_NAME = os.environ.get("TRAIN_MODEL_NAME", "Qwen/Qwen3.5-9B")
DATA_DIR = Path(os.environ.get("TRAIN_DATA_DIR", f"/opt/training/datasets/oracle_qwen35_thinkers/{PERSONA_NAME}"))
OUTPUT_DIR = Path(os.environ.get("TRAIN_OUTPUT_DIR", f"/opt/training/models/{PERSONA_NAME}-qwen35-thinker-v1"))
...
ROLE_PROMPT = os.environ.get(
    "TRAIN_ROLE_PROMPT",
    (
        "You are a Zelda ROM hacking assistant specialized in 65816 assembly, Oracle of Secrets, "
        "tool-driven debugging, and grounded technical reasoning. Stay precise, inspect evidence first, "
        "and write code that respects register width, bank boundaries, and actual engine behavior."
    ),
)


def default_pid_path(output_dir: Path) -> Path:
    runtime_root = output_dir.expanduser().resolve().parent.parent
    return runtime_root / "logs" / f"{output_dir.name}.pid"


PID_PATH = Path(os.environ.get("TRAIN_PID_PATH", str(default_pid_path(OUTPUT_DIR))))


def setup_logging() -> logging.Logger:
    logger = logging.getLogger("train_oracle_qwen35_thinker")
    if logger.handlers:
        return logger
...
def pick_precision(torch: Any) -> tuple[Any, bool, bool]:
...
def main() -> None:
    ...
    logger.info("=== Oracle Qwen 3.5 Thinker Training ===")
    logger.info("Track: %s", TRACK_NAME)
    logger.info("Persona: %s", PERSONA_NAME)
    logger.info("Model: %s", MODEL_NAME)
    logger.info("Data dir: %s", DATA_DIR)
    logger.info("Output dir: %s", OUTPUT_DIR)
    logger.info("PID path: %s", PID_PATH)
    ...
    torch_dtype, use_bf16, use_fp16 = pick_precision(torch)
