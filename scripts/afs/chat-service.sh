...
ensure_litellm_config() {
  ...
  (
    ...
    local config_tmp=""
    local legacy_tmp=""
    local generated_marker="# Generated by chat-service.sh. Changes are replaced on the next config refresh."
    trap '[[ -z "$config_tmp" ]] || rm -f "$config_tmp"; [[ -z "$legacy_tmp" ]] || rm -f "$legacy_tmp"' EXIT

    if [[ -L "$LITELLM_CONFIG_FILE" || ( -e "$LITELLM_CONFIG_FILE" && ! -f "$LITELLM_CONFIG_FILE" ) ]]; then
      echo "error: refusing to replace non-regular LiteLLM config: $LITELLM_CONFIG_FILE" >&2
      return 1
    fi

    config_tmp="$(mktemp "${LITELLM_CONFIG_FILE}.tmp.XXXXXX")"
    legacy_tmp="$(mktemp "${LITELLM_CONFIG_FILE}.legacy.XXXXXX")"
    cat <<'EOF' > "$config_tmp"
...
EOF
    if [[ ! -f "$LITELLM_CONFIG_FILE" ]]; then
      mv "$config_tmp" "$LITELLM_CONFIG_FILE"
      config_tmp=""
      echo "created"
      return 0
    fi
    ...
    local first_line=""
    IFS= read -r first_line < "$LITELLM_CONFIG_FILE" || true
    if [[ "$first_line" == "$generated_marker" ]]; then
      chmod 600 "$LITELLM_CONFIG_FILE"
      if ! cmp -s "$config_tmp" "$LITELLM_CONFIG_FILE"; then
        mv "$config_tmp" "$LITELLM_CONFIG_FILE"
        config_tmp=""
        echo "updated"
      else
    ...
    if cmp -s "$legacy_tmp" "$LITELLM_CONFIG_FILE"; then
      mv "$config_tmp" "$LITELLM_CONFIG_FILE"
      config_tmp=""
      echo "updated"
      return 0
    ...
    chmod 600 "$LITELLM_CONFIG_FILE"
    if grep -Eq 'claude-3-5|gemini-1\\.5|gemini-3-pro-preview' "$LITELLM_CONFIG_FILE"; then
      echo "warning: preserving unmanaged $LITELLM_CONFIG_FILE; it contains retired model IDs" >&2
    fi
    echo "preserved"
  )
}
...
litellm_should_run() {
  local mode="${1:-simple}"
  [[ "$mode" == "simple" ]] || return 1
  [[ -n "$AFS_ANTHROPIC_API_KEY" || -n "$AFS_GEMINI_API_KEY" || -n "$AFS_LITELLM_MASTER_KEY" || -n "$AFS_LITELLM_API_KEY" ]]
}
...
cmd_start() {
  ...
  local litellm_config_status=""
  local litellm_was_running=0

  echo "Starting AFS Chat infrastructure..."
  load_secrets

  if litellm_should_run "$mode"; then
    litellm_enabled=1
    if check_container "afs-chat-litellm"; then
  ...
  if (( litellm_enabled )); then
    sync_litellm_secrets
    litellm_config_status="$(ensure_litellm_config)"
  fi

  sync_openwebui_secrets "$litellm_enabled"
  ...
  case "$mode" in
    simple)
      ...
      if (( litellm_was_running )) && [[ "$litellm_config_status" == "created" || "$litellm_config_status" == "updated" ]]; then
        echo "  Warning: LiteLLM was already running when its catalog changed." >&2
        echo "  Run '$0 restart simple' when convenient to load the refreshed catalog." >&2
      fi
