...
def test_oracle_main_eval_plan_requires_remote_target(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="remote target"):
        build_zelda_eval_plan("oracle_main_27b_v1", local_run_dir=tmp_path)


def test_oracle_main_eval_plan_builds_remote_eval_command(tmp_path: Path) -> None:
    target = RemoteRunTarget(host="ssh6.vast.ai", port=22, remote_dir="/workspace/training")
    plan = build_zelda_eval_plan("oracle_main_27b_v1", local_run_dir=tmp_path, remote_target=target)
...
def test_legacy_switchhook_eval_alias_still_builds_remote_command(tmp_path: Path) -> None:
    target = RemoteRunTarget(host="ssh6.vast.ai", port=22, remote_dir="/workspace/training")
